...
orderly_resource <- function(files) {
  ...
  if (is.null(p)) {
    ...
    orderly2::outpack_packet_file_mark(p, files_expanded, "immutable")
...
orderly_dependency <- function(name, query, use) {
  ...
  ctx <- orderly_context()
  subquery <- NULL
  query <- orderly2::outpack_query(query, name = name, subquery = subquery)
  if (ctx$is_active) {
    orderly2::outpack_packet_use_dependency(ctx$packet, query, use,
                                           ctx$search_options)
  } else {
    id <- orderly2::outpack_search(query, parameters = ctx$parameters,
    ...
    orderly2::outpack_copy_files(id, use, ctx$path, allow_remote, ctx$root)
  }
...
orderly_global_resource <- function(...) {
  ...
  if (ctx$is_active) {
    orderly2::outpack_packet_file_mark(ctx$packet, files$here, "immutable")

---
title: "Orderly plugins"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Orderly plugins}
...
With the yaml-less design of `orderly2` (see `vignette("migrating")` if you are familar with `orderly1`), the line between a plugin and just package code is fairly blurred, but reasons for writing a plugin are typically that you want to make something easier in reports, and you want that action reflected in the orderly metadata.
...
writeLines(c(
  'example.db::query(query = "SELECT * FROM mtcars WHERE cyl == 4", as = "dat")',
...
Our plugin is called `example.db` and is listed within the `plugins` section. Then there is a top-level `example.db` field which contains configuration for our plugin; in this case indicating the path where the SQLite file can be loaded from.
...
(2) A function for serialising the results of running any functions from `orderly.R`; for now we leave that as an empty function (we may change this to make it optional?)
...
(3) The function that actually does the query; in our example above this is `example.db::query`:

```{r}
query <- function(query, as) {
...
```
The arguments here are whatever you want the user to provide. The important function here to call is `orderly2::orderly_plugin_context` which returns information that you can use to make the plugin work. This is explained in the orderly docs, but in this example we use two elements:

* `config`: the configuration fo this plugin (i.e., the processed dat from `db_config`
* `env`: the environment that the current packet is using

We modify the packet environment (by writing to `ctx$env` and return `NULL` for now.

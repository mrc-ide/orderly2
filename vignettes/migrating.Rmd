...
Sometimes, you will want to use the old version of orderly without migrating any source, for example to rerun something as it was run in 2003.  You can do this by installing `orderly1`, which can be safely installed alongside `orderly2`.  You should, however, avoid loading both packages at once (i.e., with `library()`) as this will cause ambiguity in a number of functions, notably `orderly_run()`
...
If you try to use `orderly2` with version 1 sources, then you will get an error message that will direct you to this document to decide the best way forward.

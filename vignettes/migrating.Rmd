...
The most obvious user-facing change is that there is (almost) no [YAML](https://en.wikipedia.org/wiki/YAML), with the definition of inputs and outputs for a report now defined within an R script (`<reportname>.R`).  So an orderly report that previously had an `orderly.yml` file that looked like
...
would end up within an `<reportname>.R` script that looks like:

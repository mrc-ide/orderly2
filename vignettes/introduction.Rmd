...
That's it! Notice that the initial script is just a plain R script, and you can develop it interactively from within the `src/data` directory. Note however, that any paths referred to within will be relative to `src/data` and **not** the orderly repository root. This is important as all reports only see the world relative to their `orderly.R` file.
